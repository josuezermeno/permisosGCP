...
OUTPUT_FILE="permisos_bigquery_definitivo.csv"
...
do
  ...
  # --- 1. CAPTURAR PERMISOS HEREDADOS DEL PROYECTO ---
  echo "  -> Obteniendo permisos heredados del proyecto..."
  gcloud projects get-iam-policy "$project_id" --format=json | \
  jq -r --arg project "$project_id" \
  '.bindings[] | select(.role | contains("bigquery")) | .members[] as $member | [$project, "N/A (Heredado)", "HEREDADO", ($member | split(":"))[0], ($member | split(":"))[1], .role, "PROYECTO_COMPLETO"] | @csv' >> "$OUTPUT_FILE"

  # --- 2. CAPTURAR PERMISOS DIRECTOS POR DATASET ---
  datasets=$(bq ls --project_id="$project_id" | tail -n +3 | awk '{print $1}' | grep -v '^bq_read_all')
...
echo ""
echo "========================================================================"
echo "✅ Análisis detallado finalizado."
